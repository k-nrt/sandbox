...
public class StarField
{
	...
	*/
	...
	private DelayResourceQueueMarker m_markerReady = new DelayResourceQueueMarker("StarField");
	public static class RenderContext
	{
		public static final int kHistories = 8;
		public float[] Projections = new float[kHistories * 16];
		public float[] WorldViews = new float[kHistories * 16];

		public float[] Intensities = new float[kHistories];
		public float[] ElapsedTimes = new float[kHistories];
		...
		{
			for( int i = (kHistories-2) ; i >= 0 ; i-- )
			{
				for( int j = 0 ; j < 16 ; j++ )
				{
					WorldViews[(i+1)*16+j] = WorldViews[i*16+j];
					Projections[(i+1)*16+j] = Projections[i*16+j];
				}
				ElapsedTimes[i+1] = ElapsedTimes[i];
			}

			for( int i = 0 ; i < 16 ; i++ )
			{
				WorldViews[i] = worldView[i];
				Projections[i] = projection[i];
			}		
			ElapsedTimes[0] = fElapsedTime;

			float fTimeOut = 4.0f/60.0f;
			float fTime = 0.0f;
			for( int i = 0 ; i < kHistories ; i++ )
			{
				Intensities[i] = 1.0f - (fTime/fTimeOut);
				fTime += ElapsedTimes[i];
			}
		}
	}
		...
		//throws ThreadForceDestroyException
	{
		...
		int nbVertices = m_nbStars * RenderContext.kHistories;
		int nbStride = 16;

		m_eLineIndexFormat = GLES20.GL_UNSIGNED_SHORT;
		int nbIndices = m_nbLineIndices = m_nbStars * (RenderContext.kHistories - 1) * 2;
		...
		for (int i = 0 ; i < m_nbStars ; i++)
		{
			...
			int start = (i*RenderContext.kHistories);
			
			for (int j = 0 ; j < RenderContext.kHistories ; j++)
			{
			...
			for( int j = 0 ; j < (RenderContext.kHistories-1) ; j++ )
			{
	...
	*/
	void Render(BasicRender basicRender, final RenderContext rc )
	{
		if(m_markerReady.Done==false)
		{
			return;
		}
		
		//Render r = basicRender.GetRender();
		GfxCommandContext r = SubSystem.GfxCommandContext;
		...
		r.SetMatrixArray(u_projections, rc.kHistories, rc.Projections, 0);
		r.SetMatrixArray(u_worldViews, rc.kHistories, rc.WorldViews, 0);
		
		r.SetMatrix(u_viewInverse, matrixViewInverse.Values );
		
		r.SetFloatArray(u_fIntensities,rc.kHistories,rc.Intensities,0);
		...
		r.DrawElements( Primitive.Lines, m_nbLineIndices, IndexFormat.UnsignedShort, 0 );
		...
		r.DrawArrays( Primitive.Points, 0, m_nbStars );
