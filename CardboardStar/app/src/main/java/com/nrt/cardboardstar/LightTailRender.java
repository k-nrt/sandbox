...
class LightTailRender
{
	...
	public DelayResourceQueueMarker Marker = new DelayResourceQueueMarker("LightTailRender");
	...
	*/
	...
	public void Render( BasicRender br, LightTail lightTail, Frustum frustum, float fScale, Float4 v4ColorIn, Float4 v4ColorOut )
	{
		...
		gfxc.SetProgram( Program );

		gfxc.SetMatrix( u_m4WorldViewProjection, mc.GetWorldViewProjection().Values );
		gfxc.SetFloat( u_fRadius, 1.0f );
		gfxc.SetFloat3( u_v3ViewPosition, mc.GetViewInverse().GetAxisW( Float3.Local()) );
		gfxc.SetFloat4( u_v4ColorIn, v4ColorIn );
		gfxc.SetFloat4( u_v4ColorOut, v4ColorOut );
		...
		for (int i = 0 ; i < lightTail.Length ; i += 15)
		{
			...
			gfxc.SetFloat4Array( u_v4TransformsX, m_arrayTransformsX );
			gfxc.SetFloat4Array( u_v4TransformsY, m_arrayTransformsY );
			gfxc.SetFloat4Array( u_v4TransformsZ, m_arrayTransformsZ );
				
			gfxc.SetVertexBuffer( VertexBuffer );
			gfxc.EnableVertexStream( VertexStream, 0 );
			...
			gfxc.DrawElements
			(
				Primitive.Triangles,//	GLES20.GL_TRIANGLES,
				6 * WeightCount * PolygonCount * (nbTransforms - 1),
