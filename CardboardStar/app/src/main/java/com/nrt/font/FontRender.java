...
public class FontRender
{
	...
	public BlendState m_blendState = null;
	public DepthStencilState m_depthStencilState = null;
	...
	public FontRender( DelayResourceQueue drq, BasicRender basicRender )// throws ThreadForceDestroyException
	{
		...
		m_blendState = new BlendState( true, BlendFunc.SrcAlpha, BlendFunc.OneMinusSrcAlpha );
		
		//GLES20.glDepthFunc( GLES20.GL_LEQUAL );
		m_depthStencilState = new DepthStencilState(true, DepthFunc.LessEqual, false);
	}
	...
	public void Begin()
	{
		...
		m_vertexOffset = br.GetVertexBuffer().GetOffset();
		m_indexOffset = br.GetIndexBuffer().GetOffset();
	...
	{
		FrameLinearVertexBuffer vb = m_basicRender.GetVertexBuffer();
		FrameLinearIndexBuffer ib = m_basicRender.GetIndexBuffer();
	...
	public void End()
	{
		...
		GfxCommandContext gfxc = m_basicRender.GetCommandContext();

		//GLES20.glEnable( GLES20.GL_BLEND );
		//GLES20.glBlendFunc( GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA );
		//GLES20.glDepthFunc( GLES20.GL_LEQUAL );
		gfxc.SetBlendState( m_blendState );
		
		gfxc.SetProgram( m_program );

		gfxc.SetMatrix( u_worldViewProjection, m_basicRender.GetMatrixCache().GetWorldViewProjection().Values );
		/*
		 ...
		 */
		gfxc.SetVertexBuffer( m_basicRender.GetVertexBuffer() );
		gfxc.EnableVertexStream( m_stream, m_vertexOffset );

		gfxc.SetIndexBuffer( m_basicRender.GetIndexBuffer() );
		//GLES20.glDrawElements( GLES20.GL_TRIANGLES, m_nbVertices, GLES20.GL_UNSIGNED_SHORT, nIndexOffset );
		gfxc.DrawElements( Primitive.Triangles, m_nbVertices, IndexFormat.UnsignedShort, m_indexOffset );
