...
public class OcclusionQuery
{
	...
	public DelayResourceQueueMarker Marker = new DelayResourceQueueMarker("OcclusionQuery");
	...
	*/
	...
	public BlendState m_blendStateColorMask = new BlendState
	(
		false, BlendFunc.One, BlendFunc.Zero, 
		true,false,false,true
	);
	
	public DepthStencilState m_depthStencilStateTestEnableWriteDisable =
		new DepthStencilState( true, DepthFunc.Less, false );
	
	public void RenderPointsToFrameBuffer
	( 
		GfxCommandContext gfxc, 
		MatrixCache mc, 
		FrameLinearVertexBuffer vb,
		...
		)
	{
		...
		if( gfxc == null || mc == null || vb == null || frameBuffer == null )
		{
			return;
		}
		...
		gfxc.SetBlendState( m_blendStateColorMask );
		//GLES20.glEnable( GLES20.GL_DEPTH_TEST );
		//GLES20.glDepthMask( false );
		gfxc.SetDepthStencilState( m_depthStencilStateTestEnableWriteDisable );

		gfxc.SetProgram( m_program );
		gfxc.SetMatrix( u_matrixWorldViewProjection, mc.GetWorldViewProjection().Values );
		gfxc.SetFloat( u_fPointSize, PointSize );
		gfxc.SetFloat4( u_f4Color, 1.0f, 1.0f, 1.0f, 1.0f );

		gfxc.SetVertexBuffer( vb );
		gfxc.EnableVertexStream( m_vertexStream, vbOffset );

		gfxc.DrawArrays( Primitive.Points, 0, nbVertices );

		gfxc.DisableVertexStream(m_vertexStream);	
