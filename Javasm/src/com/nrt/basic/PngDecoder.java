...
public class PngDecoder
{
	...
	{		
		...
		while (true)
		{
			...
			if (view != null)
			{
				view.append(type + " " + Integer.toString(length) + "\n");
			}
			...
			if (type.compareTo("IHDR") == 0)
			{
				...
				if (view != null)
				{
					view.append(String.format("%dx%d %d %s %d %d %d\n",
			...
			else if (type.compareTo("IDAT") == 0)
			{
				...
				if (inflater.needsInput() == false)
				{
					...
					if (view != null)
					{
						view.append(String.format("inflate %d/%d\n", pos + size, decompressedData.length));
					}
				}
			}
		...
		if (view != null)
		{
			view.append(String.format("decompressed\n"));
		}
