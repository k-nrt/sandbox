...
class SimpleParticle
{
	...
	public void Render()
	{
		...
		RingVertexBuffer rvb = br.GetVertexBuffer();
		RingIndexBuffer ivb = br.GetIndexBuffer();
		/*
		int nbParticles = 0;
		for( int i = 0 ; i < m_particles.length ; i++ )
		{
		...
		*/
		...
		for( int i = 0 ; i < m_particles.length ; i++ )
		{
			...
			for( int j = 0 ; j < 4 ; j++ )
			{
				rvb.Add( particle.m_f3Position.X,
					particle.m_f3Position.Y,
					particle.m_f3Position.Z,
					(float) j );
					
				rvb.Add( particle.m_fU, particle.m_fV,
					particle.m_fAlpha, particle.m_fRadius );
					
				rvb.Add( particle.m_fAngle );
			}
			
			ivb.Add( (short) (nbVertices + 0) );
			ivb.Add( (short) (nbVertices + 1) );
			ivb.Add( (short) (nbVertices + 2) );
			ivb.Add( (short) (nbVertices + 0) );
			ivb.Add( (short) (nbVertices + 2) );
			ivb.Add( (short) (nbVertices + 3) );
			nbVertices += 4;
			nbIndices += 6;
		}
		
		int nVertexOffset = rvb.End();
		int nIndexOffset = ivb.End();
