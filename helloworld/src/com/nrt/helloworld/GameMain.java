...
//	
	...
	private void RenderScene()
	{
		...
		ModelShadow shadow = new ModelShadow();
		{
			...
			mr.Draw(m_modelEnemy0, m_poseStack.ModelMatrices, ModelShaderSet.EShadow.Caster);
			GLES20.glCullFace(GLES20.GL_BACK);
		}		
		...
		for (GameStarShip starShip : m_starShips)
		{
			...
			if (starShip.Model.IsVible())
			{
				...
				mr.Draw(starShip.Model.Model, ModelShaderSet.EShadow.Caster);
...

		{
			...
			mr.Draw(m_modelEnemy0, m_poseStack.ModelMatrices, ModelShaderSet.EShadow.Reciever);
		}
		...
		for (GameStarShip starShip : m_starShips)
		{
			...
			if (starShip.IsBusy() || starShip.IsExplosion())
			{
				...
				mr.Draw(starShip.Model.Model, ModelShaderSet.EShadow.Reciever);
			}
			else if (starShip.IsExplosionFinal())
			{
				...
				mr.Draw(starShip.Model.Model, ModelShaderSet.EShadow.Disable);
		...
		for (GameEnemy  enemy : m_enemies)
		{
			if (enemy.IsVisible())
			{
				...
				{
					mc.SetWorld(matrix);
					SubSystem.ModelRender.Draw(model, m_poseStack.ModelMatrices, ModelShaderSet.EShadow.Disable);
		...
		SubSystem.ModelRender.Draw(m_modelEnemy0, m_poseStack.ModelMatrices, ModelShaderSet.EShadow.Caster);
