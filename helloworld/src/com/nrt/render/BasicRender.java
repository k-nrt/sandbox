...
public class BasicRender
{
	...
	//throws ThreadForceDestroyException
	{
		...
		m_vertexBuffer = new RingVertexBuffer( SubSystem.DelayResourceQueue, nVertexBufferSize);
		m_indexBuffer = new RingIndexBuffer( SubSystem.DelayResourceQueue, nIndexBufferSize);
		...
		(
			SubSystem.DelayResourceQueue,
			abColors,
			new VertexShader( SubSystem.DelayResourceQueue, strVsColor),
			new FragmentShader( SubSystem.DelayResourceQueue, strFsColor)
		);
		...
		(
			SubSystem.DelayResourceQueue, 
			abColorTextures,
			new VertexShader(SubSystem.DelayResourceQueue,strVsColorTexture),
			new FragmentShader(SubSystem.DelayResourceQueue,strFsColorTexture)
		);
	...
	public void Begin(int primitive, EShader shader)
	{
...
//		r.DisableVertexStream(m_streams[m_eShader.ordinal()]);
	...
	{
		Begin(GLES20.GL_TRIANGLE_STRIP, EShader.Color);
	...
	{
		Begin(GLES20.GL_LINE_STRIP, EShader.Color);
		SetVertex(rect.X, rect.Y, 0.0f);
		SetVertex(rect.X + rect.Width, rect.Y, 0.0f);
		SetVertex(rect.X + rect.Width, rect.Y + rect.Height, 0.0f);
	...
	{
		Begin( GLES20.GL_LINES, EShader.Color );
